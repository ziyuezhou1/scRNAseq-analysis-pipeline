...
as <- FindIntegrationAnchors(seu, verbose = TRUE, dims = seq_len(50))
seu <- IntegrateData(anchorset = as, dims = seq_len(50), verbose = TRUE)

# Scale integrated data
DefaultAssay(seu) <- "integrated"
seu <- ScaleData(seu, display.progress = FALSE)
...
# CLUSTERING
seu <- FindNeighbors(seu, reduction = "pca", dims = seq_len(18), verbose = FALSE)
...
# SAVE SeuratObject
saveRDS(seu, file.path("./", "seu_18PC.rds"))
