...
out_dir <- "./outs_split_condition/"
out_files <- list.files(out_dir, full.names = TRUE)


generate_heatmaps <- function(x){
  seurat_path <- paste0("so.renamed_res0.2_", x, ".rds")
  out_dir <- paste0("./outs_split_condition/", x)
  if (!dir.exists(out_dir)) {dir.create(out_dir)}
  
  analysis <- paste0("so.renamed_res0.2_", x)
  ### Load pySCENIC output loom file (regulons)

  # from pySCENIC working directory:

  path2loom <- out_files[grep(paste0("pySCENIC_viz_", x, ".loom"), out_files)]
  ...
  motifEnrichment <- read.csv(file = paste0("./outs_split_condition/pySCENIC_CTX_regulons_", x, ".csv"), header = TRUE, skip = 1)
  ...
  pdf(file = paste0(out_dir, "/", analysis,"_pyscenic_regulon_activity_by_celltype.pdf"),
    width = 10,
    height = 12)
  ...
  htmlwidgets::saveWidget(as_widget(fig), 
                          paste0(out_dir, "/", analysis, "_pyscenic_cluster_specific_regulons.html"))
  options(repr.plot.width=5, repr.plot.height=5) # To set the figure size in Jupyter
  CellType <- levels(Idents(seurat_rna))
  for (i in CellType) {
    pdf(paste0(out_dir, "/", analysis, gsub("/", "_", i), "_specific_regulon.pdf"),
        width = 7,
        height = 7)
