# loading data
# SD1D vs. NS
sc_de <- read.csv("./SD1D vs ZT6 pseudobulk.csv")
bulk_de <- read.csv("./NS_Control_vs_SD1D_Experiment-ExpDiff_with_gene_names.csv")
...
calculate_overlap = function(bulk_de, sc_de,
  ...
  # remove NAs
  sc_de %<>% filter(!is.na(p_val), !is.na(p_val_adj))
  bulk_de %<>% filter(!is.na(p_val), !is.na(p_val_adj))
  ...
  if (method == 'fcc') {
    ...
    k = as.integer(k)
    ## rank in descending order first by p_val_adj
    vec1 = bulk_de %>%
      arrange(p_val_adj) %>%
      pull(gene) %>%
      head(k)
