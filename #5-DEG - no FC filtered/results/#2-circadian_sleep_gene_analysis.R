...
GO_analysis <- function(x){
  # loading data
  N2_vs_N1 <- read.csv(paste0("./N2_vs_N1_re/N2_vs_N1_re_", x, ".csv"))
  N3_vs_N1 <- read.csv(paste0("./N3_vs_N1_re/N3_vs_N1_re_", x, ".csv"))
  N3_vs_N2 <- read.csv(paste0("./N3_vs_N2_re/N3_vs_N2_re_", x, ".csv"))
  # subset padj <= 0.05
...
all$group <- factor(all$group, levels = c("Exc-neuron","Inh-neuron","Astro","Micro","Oligo","OPC"))
...
re@compareClusterResult$hit <- as.numeric(sub("\\/.*", "", re@compareClusterResult$GeneRatio))
re@compareClusterResult$geneset <- as.numeric(sub("\\/.*", "", re@compareClusterResult$BgRatio))
re@compareClusterResult$EnrichRatio <- re@compareClusterResult$hit / re@compareClusterResult$geneset
png("./circadian_sleep_BP_all_compare_new_using_EnrichRatio_20231128.png",
    width = 2400,
    height = 1400)
ggplot(re, 
  ...
  geom_point(aes(size = EnrichRatio, color = p.adjust)) +
  ...
  theme(text = element_text(size = 28),
        ...
        axis.text.x = element_text(size = 40, angle = 30, vjust = 0.95, hjust = 0.95),
