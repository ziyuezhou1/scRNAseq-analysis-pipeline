...
p + scale_color_manual(values=c("#999999", "#386087", "#9A1200")) +
  theme_classic() +
  theme(text = element_text(size = 20), plot.title = element_text(hjust = 0.5)) +
  labs(title = "DEG N5 vs. N4 renamed", x = "Clusters", y = "log2FC(N5/N4)")
dev.off()
...
for (i in seq.int(1,4,1)){
  write.csv(N5_VS_N4_re[[i]], file = paste0(".\\results\\N5_vs_N4_re\\N5_vs_N4_re_merge_glial_", to_name_csv[[i]], ".csv"))
}
df_re <- as.data.frame(do.call(rbind, N5_VS_N4_re))
df_re$names <- row.names(df_re)
df_re <- df_re %>% 
...
png("./figures/N5_vs_N4_re_merge_glial.png",
    width = 2500,
    height = 800)
p <- ggplot(df_re, aes(x = groups, y = avg_log2FC, color = threshold)) +
  geom_jitter(size = 2, width = 0.2)
