...
for (i in levels(Idents(seurat.renamed))){
  N5_VS_N1_re[[i]] <- FindMarkers(object = seurat.renamed,
                                  group.by = "group_id",
                                  ident.1 = "SD3D R7",
                                  ident.2 = "ZT6",
                                  subset.ident = i,
...
if (!file.exists("results/N5_vs_N1_re")) dir.create("results/N5_vs_N1_re")
for (i in seq.int(1,8,1)){
  write.csv(N5_VS_N1_re[[i]], file = paste0(".\\results\\N5_vs_N1_re\\N5_vs_N1_re_", to_name_csv[[i]], ".csv"))
}
df_re <- as.data.frame(do.call(rbind, N5_VS_N1_re))
...
p + scale_color_manual(values=c("#999999", "#386087", "#9A1200")) +
  theme_classic() +
  theme(text = element_text(size = 20), plot.title = element_text(hjust = 0.5)) +
  labs(title = "DEG N5 vs. N1 renamed", x = "Clusters", y = "log2FC(N5/N1)")
dev.off()
...
for (i in seq.int(1,4,1)){
  write.csv(N5_VS_N1_re[[i]], file = paste0(".\\results\\N5_vs_N1_re\\N5_vs_N1_re_merge_glial_", to_name_csv[[i]], ".csv"))
}
df_re <- as.data.frame(do.call(rbind, N5_VS_N1_re))
df_re$names <- row.names(df_re)
df_re <- df_re %>% 
...
png("./figures/N5_vs_N1_re_merge_glial.png",
    width = 2500,
    height = 800)
p <- ggplot(df_re, aes(x = groups, y = avg_log2FC, color = threshold)) +
  geom_jitter(size = 2, width = 0.2)
